...
indata <- indata %>%
  mutate(Date = mdy(BLANK)) %>% # convert "Date" to a date object with mdy()
...
```{r define_year}
PQ <- indata %>%
  BLANK(wtr_yr == 2006) # extract the 2006 water year with filter()

# plot discharge for 2006
ggplot(BLANK, aes(x = Date, y = Discharge_mm)) +
  geom_line()
...
**1) QUESTION (3 pts): Interpret the two figures. What is the meaning of "frac"? 
...
# this is the "raw" transfer function. This is eq 13 from Weiler 2003. Use the time step for tau. The Gamma function in R is gamma(). NOTE THAT THIS EQ IN WEILER ET AL IS WWRONG! It's exp(-tau / b) and not divided by alpha.
...
ggplot(tf_plot, aes(tau, gTF)) +
  geom_line()
```
**2) QUESTION (2 pt): Why is it important to normalize the transfer function?**  
ANSWER: 

**3) QUESTION (4 pt): Describe the transfer function. What are the units and what does the shape of the transfer function mean for runoff generation?**  
ANSWER: 
...
toc()
```
**4) QUESTION (5 pts): We set the TF length to 365. What is the physical meaning of this (i.e., how well does this represent a real system and why)? Could the transfer function be shorter or longer than that?**  
...
# plot hydrographs (as lines) and label the y-axis. Here, we are setting
# axis limits with lims()
ggplot(data = PQ_long, aes(x = Date, y = value, color = key)) +
  geom_line() +
  labs(x = {}, y = "Q (mm/day)", color = {}) +
  BLANK(x = as.Date(c("2005-10-01", "2006-09-30")))
```
**5) QUESTION (3 pt): Evaluate how good or bad the model performed (i.e., visually compare simulated and observed streamflow, e.g., low flows and peak flows).**  
ANSWER:

**6) QUESTION (2 pt): Compare the effective precipitation total with the simulated runoff total and the observed runoff total. What is p_eff and how is it related to q_all? Discuss why there is a (small) mismatch between the sums of p_eff and q_all.**    
ANSWER:  
